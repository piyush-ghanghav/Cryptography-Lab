...
def enc(card_number, rounds, produce_keys):
    ...
    if produce_keys:
        for i in range(rounds):
            key = binascii.hexlify(Random.new().read(16)).decode('utf-8')  # Use binascii.hexlify to convert to hex
            round_keys.append(key)

    for i in range(rounds):        
        
        key = round_keys[i]
        obj = AES.new(binascii.unhexlify(key), AES.MODE_ECB)  # Use AES.MODE_ECB mode
        hex_r = '{:08x}'.format(int(r + '0'*5, 2))
        b = hex_r + '0'*23 + str(i+1)
        enc_res = binascii.hexlify(obj.encrypt(binascii.unhexlify(b[:32]))).decode('utf-8')[:7] + '0'
        tmp = r
        r = xor(enc_res, l+'0'*5)
        l = tmp
